...
/**
  ...
  * 返 回 值：无
  */
void OLED_WriteCommand(I2C_HandleTypeDef *hi2cx, uint8_t Command)
{
	uint8_t buffer[2];
	buffer[0] = 0x00;		//控制字节，给0x00，表示即将写命令
	buffer[1] = Command;	//命令字节
	
	// 使用HAL I2C库发送命令
	HAL_I2C_Master_Transmit(hi2cx, 0x78, buffer, 2, 100);
}
...
/**
  ...
  * 返 回 值：无
  */
void OLED_WriteData(I2C_HandleTypeDef *hi2cx,uint8_t *Data, uint8_t Count)
{
	static uint8_t buffer[129];	// 129字节静态缓冲区：1字节控制 + 最多128字节数据
	buffer[0] = 0x40;			// 控制字节，给0x40，表示即将写数据
	
	// 复制数据到缓冲区
	memcpy(&buffer[1], Data, Count);
	
	// 使用HAL I2C库发送数据
	HAL_I2C_Master_Transmit(hi2cx, 0x78, buffer, Count + 1, 100);
}
...
void OLED_Init(I2C_HandleTypeDef *hi2cx)
{
	...
	OLED_WriteCommand(hi2cx, 0xAE);	//设置显示开启/关闭，0xAE关闭，0xAF开启
	
	OLED_WriteCommand(hi2cx, 0xD5);	//设置显示时钟分频比/振荡器频率
	OLED_WriteCommand(hi2cx, 0x80);	//0x00~0xFF
	
	OLED_WriteCommand(hi2cx, 0xA8);	//设置多路复用率
	OLED_WriteCommand(hi2cx, 0x3F);	//0x0E~0x3F
	
	OLED_WriteCommand(hi2cx, 0xD3);	//设置显示偏移
	OLED_WriteCommand(hi2cx, 0x00);	//0x00~0x7F
	
	OLED_WriteCommand(hi2cx, 0x40);	//设置显示开始行，0x40~0x7F
	
	OLED_WriteCommand(hi2cx, 0xA1);	//设置左右方向，0xA1正常，0xA0左右反置
	
	OLED_WriteCommand(hi2cx, 0xC8);	//设置上下方向，0xC8正常，0xC0上下反置

	OLED_WriteCommand(hi2cx, 0xDA);	//设置COM引脚硬件配置
	OLED_WriteCommand(hi2cx, 0x12);
	
	OLED_WriteCommand(hi2cx, 0x81);	//设置对比度
	OLED_WriteCommand(hi2cx, 0xCF);	//0x00~0xFF

	OLED_WriteCommand(hi2cx, 0xD9);	//设置预充电周期
	OLED_WriteCommand(hi2cx, 0xF1);

	OLED_WriteCommand(hi2cx, 0xDB);	//设置VCOMH取消选择级别
	OLED_WriteCommand(hi2cx, 0x30);

	OLED_WriteCommand(hi2cx, 0xA4);	//设置整个显示打开/关闭

	OLED_WriteCommand(hi2cx, 0xA6);	//设置正常/反色显示，0xA6正常，0xA7反色

	OLED_WriteCommand(hi2cx, 0x8D);	//设置充电泵
	OLED_WriteCommand(hi2cx, 0x14);

	OLED_WriteCommand(hi2cx, 0xAF);	//开启显示

	OLED_Clear();				//清空显存数组
	OLED_Update(hi2cx);				//更新显示，清屏，防止初始化后未显示内容时花屏
}
...
/**
  ...
  */
void OLED_SetCursor(I2C_HandleTypeDef *hi2cx, uint8_t Page, uint8_t X)
{
...
//	X += 2;
	...
	/*通过指令设置页地址和列地址*/
	OLED_WriteCommand(hi2cx, 0xB0 | Page);					//设置页位置
	OLED_WriteCommand(hi2cx, 0x10 | ((X & 0xF0) >> 4));	//设置X位置高4位
	OLED_WriteCommand(hi2cx, 0x00 | (X & 0x0F));			//设置X位置低4位
}
...
/**
  ...
  */
void OLED_Update(I2C_HandleTypeDef *hi2cx)
{
	uint8_t j;
	/*遍历每一页*/
	for (j = 0; j < 8; j ++)
	{
		/*设置光标位置为每一页的第一列*/
		OLED_SetCursor(hi2cx, j, 0);
		/*连续写入128个数据，将显存数组的数据写入到OLED硬件*/
		OLED_WriteData(hi2cx, OLED_DisplayBuf[j], 128);
	}
}
...
/**
  ...
  *           故调用显示函数后，要想真正地呈现在屏幕上，还需调用更新函数
  */
void OLED_UpdateArea(I2C_HandleTypeDef *hi2cx, int16_t X, int16_t Y, uint8_t Width, uint8_t Height)
{
	int16_t j;
	int16_t Page, Page1;
	...
	for (j = Page; j < Page1; j ++)
	{
		...
		{
			/*设置光标位置为相关页的指定列*/
			OLED_SetCursor(hi2cx, j, X);
			/*连续写入Width个数据，将显存数组的数据写入到OLED硬件*/
			OLED_WriteData(hi2cx, &OLED_DisplayBuf[j][X], Width);
		}
	}
}
